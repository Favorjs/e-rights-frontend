...
import { ArrowLeft, Receipt, CheckCircle, Eye, Download, FileText, ChevronRight, ChevronLeft, Info, Search, X, ChevronDown, AlertTriangle } from 'lucide-react';
...
const InternalSearchableSelect = ({
  ...
  return (
    <div className="relative w-full" ref={selectRef}>
      <button
        type="button"
        className={`relative w-full bg-white border border-slate-200 rounded-xl shadow-sm pl-4 pr-10 py-3 text-left cursor-default focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-600 text-sm ${className}`}
              ...
              filteredOptions.map((option, index) => {
                ...
                return (
                  <div
                    ...
                    className={`px-4 py-2.5 text-sm transition-colors cursor-pointer ${value === optionValue ? 'bg-emerald-50 text-emerald-900 font-bold' : 'text-slate-700 hover:bg-slate-50'}`}
                    onClick={(e) => {
                      e.stopPropagation();
  ...
  if (loading) return (
    <div className="App flex items-center justify-center min-h-screen bg-slate-50/50">
      <div className="text-center">
        <div className="loading-spinner h-10 w-10 mx-auto mb-4 border-emerald-600"></div>
      ...
      <div className="container-custom py-12 md:py-20">
        <div className="max-w-3xl mx-auto space-y-12">
          <div className="text-center space-y-4">
            <div className="w-20 h-20 bg-emerald-600 rounded-full flex items-center justify-center mx-auto shadow-xl shadow-emerald-200">
            ...
            <div className="p-8 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-2xl font-bold text-emerald-600">₦{parseFloat(calculateTotalPayment(submittedForm)).toLocaleString()}</p>
                </div>
                <div className="space-y-1">
        ...
        <div className="max-w-5xl mx-auto mb-12">
            ...
            {steps.map((step, index) => (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${currentStep > step.id ? 'bg-emerald-600 border-emerald-600 text-white' : currentStep === step.id ? 'bg-white border-emerald-600 text-emerald-600 shadow-md' : 'bg-white border-slate-200 text-slate-300'}`}>
                ...
                {index < steps.length - 1 && <div className={`flex-1 h-[2px] mx-2 ${currentStep > step.id ? 'bg-emerald-600' : 'bg-slate-200'}`}></div>}
              </React.Fragment>
            ))}
          </div>
          <div className="md:hidden px-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">{steps[currentStep - 1].title}</span>
            </div>
            <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-600 transition-all duration-500" style={{ width: `${(currentStep / steps.length) * 100}%` }}></div>
            </div>
          </div>
        </div>

        {/* Global Context Bar */}
        <div className="card bg-slate-900 text-white mb-8 overflow-hidden shadow-xl border-none">
          <div className="p-6 md:p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="flex items-center space-x-2 text-emerald-400">
                <Info className="h-3.5 w-3.5" />
                <span className="text-[9px] font-bold uppercase tracking-[0.2em]">Application Record</span>
              </div>
              <h2 className="text-xl md:text-2xl font-bold uppercase tracking-tight">{shareholder.name}</h2>
              <div className="flex flex-wrap items-center gap-3 opacity-60 text-[10px] uppercase font-bold tracking-widest">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 backdrop-blur-sm self-start lg:self-center w-full lg:w-auto">
              <div className="grid grid-cols-2 gap-8">
                <div>
                  <p className="text-[9px] font-bold uppercase text-emerald-400 tracking-widest mb-1">Entitlement</p>
                  <p className="text-xl font-bold">{shareholder.rights_issue.toLocaleString()}</p>
                </div>
                <div className="text-right">
                  <p className="text-[9px] font-bold uppercase text-emerald-400 tracking-widest mb-1">Status</p>
                  <p className="text-[10px] font-bold uppercase tracking-widest">Authenticated</p>
                </div>
              </div>
            </div>
          </div>
          <div className="bg-emerald-700 px-6 py-3 flex flex-col md:flex-row justify-between items-center text-[9px] font-bold uppercase tracking-widest text-emerald-50 gap-2">
            <div className="flex gap-4"><span>Providus: 1308407124</span><span>Taj Bank: 0013161672</span></div>
            <span className="italic opacity-80">Linkage Assurance Plc Rights Issue 2025</span>
          </div>
        </div>
            ...
            <div className="flex-1 space-y-8 relative z-10">
                  ...
                  <div className="p-8 bg-emerald-900 rounded-2xl text-white relative overflow-hidden">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
                      <div className="space-y-1 text-center md:text-left">
                        <span className="text-[9px] font-bold uppercase text-emerald-400 tracking-widest">Provisional Allotment</span>
                ...
                <div className="space-y-6 animate-fade-in">
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 md:p-8 max-h-[350px] overflow-y-auto custom-scrollbar">
                    <div className="prose prose-sm text-slate-600 space-y-4">
                      ...
                      <div className="pl-4 border-l-2 border-emerald-600 py-1 space-y-2">
                  ...
                  <label className="flex items-center p-5 bg-white border border-slate-200 rounded-2xl cursor-pointer hover:border-emerald-600 transition-colors">
                    <input type="checkbox" name="instructions_read" checked={formData.instructions_read} onChange={handleInputChange} className="h-5 w-5 rounded text-emerald-600" />
              ...
              {currentStep === 4 && (
                <div className="animate-fade-in grid grid-cols-1 md:grid-cols-2 gap-6">
                  {['full_acceptance', 'renunciation_partial'].map((type) => (
                    <label key={type} className={`relative flex flex-col p-6 border-2 rounded-2xl cursor-pointer transition-all ${formData.action_type === type ? 'border-emerald-600 bg-emerald-50/50' : 'border-slate-200 bg-white hover:border-slate-300'}`}>
                      <input type="radio" name="action_type" value={type} checked={formData.action_type === type} onChange={handleInputChange} className="absolute top-6 right-6 h-5 w-5 text-emerald-600" />
              ...
              {currentStep === 5 && formData.action_type === 'full_acceptance' && (
                <div className="animate-fade-in space-y-8">
                  <label className={`flex items-start p-6 border-2 rounded-2xl cursor-pointer transition-all ${formData.accept_full ? 'border-emerald-600 bg-emerald-50/30' : 'border-slate-200 bg-slate-50/30'}`}>
                    <input type="checkbox" name="accept_full" checked={formData.accept_full} onChange={handleInputChange} className="mt-1 h-5 w-5 text-emerald-600" />
                    <div className="ml-4">
                  ...
                  <label className={`flex items-start p-6 border-2 rounded-2xl cursor-pointer transition-all ${formData.apply_additional ? 'border-emerald-600 bg-emerald-50/30' : 'border-slate-200 bg-slate-50/30'}`}>
                    <input type="checkbox" name="apply_additional" checked={formData.apply_additional} onChange={handleInputChange} className="mt-1 h-5 w-5 text-emerald-600" />
                    <div className="ml-4">
                  ...
                  {formData.apply_additional && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-8 bg-slate-50 rounded-2xl border border-slate-200">
                      ...
                      <div className="space-y-2">
                        ...
                        <input type="text" value={calculatedAmount.toLocaleString()} readOnly className="input-custom bg-emerald-50 text-emerald-900 font-bold" />
                      </div>
                    </div>
                  )}

                  <div className="pt-8 space-y-6">
                    <div className="bg-emerald-900 p-8 rounded-2xl text-white flex flex-col sm:flex-row items-center justify-between gap-6">
                      <div className="text-center sm:text-left">
                        <label className="text-[9px] font-bold text-emerald-400 uppercase tracking-widest mb-1 block">Total Consideration</label>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full sm:w-auto">
                        <div className="space-y-1"><label className="label-custom text-emerald-400/50">Payment Bank</label><input type="text" name="bank_name" value={formData.bank_name} onChange={handleInputChange} className="w-full bg-white/10 border border-white/20 rounded-lg p-2 text-xs text-white font-medium" /></div>
                        <div className="space-y-1">
                          <label className="label-custom text-emerald-400/50">Cheque Number</label>
                          ...
                          <p className="text-[8px] text-emerald-400/40 uppercase font-bold mt-1 leading-tight">Only for cheque payments</p>
                        </div>
                        <div className="space-y-1"><label className="label-custom text-emerald-400/50">Branch</label><input type="text" name="branch" value={formData.branch} onChange={handleInputChange} className="w-full bg-white/10 border border-white/20 rounded-lg p-2 text-xs text-white font-medium" /></div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="animate-fade-in space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="p-5 bg-emerald-50 rounded-2xl border border-emerald-100">
                      <label className="label-custom text-emerald-600">Consideration (₦)</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <label className="flex items-center p-4 bg-white border border-slate-200 rounded-xl cursor-pointer">
                      <input type="checkbox" name="accept_partial" checked={formData.accept_partial} onChange={handleInputChange} className="h-5 w-5 text-emerald-600" />
                ...
                <div className="animate-fade-in space-y-8">
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <button
                        ...
                        className={`p-4 rounded-xl border-2 transition-all ${formData.signature_type === 'single' ? 'border-emerald-600 bg-emerald-50 text-emerald-900' : 'border-slate-200 bg-white text-slate-500'}`}
                      >
                      ...
                      <button
                        ...
                        className={`p-4 rounded-xl border-2 transition-all ${formData.signature_type === 'joint' ? 'border-emerald-600 bg-emerald-50 text-emerald-900' : 'border-slate-200 bg-white text-slate-500'}`}
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      <div className={`relative h-40 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center transition-all ${formData.receipt ? 'border-emerald-600 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                      ...
                      <div className="space-y-4">
                        {formData.signatures.map((sig, index) => (
                          <div key={index} className="relative group">
                            <div className={`relative h-32 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center transition-all ${sig ? 'border-emerald-600 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                        ...
                        {formData.signature_type === 'joint' && (
                          <button
                            ...
                            className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 hover:border-emerald-600 hover:text-emerald-600 transition-all text-[10px] font-bold uppercase tracking-widest"
                          >
                            + Add Signature
                          </button>
                        )}
                ...
                <div className="animate-fade-in space-y-12">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                      ...
                      <div className="space-y-1"><p className="text-[9px] font-bold text-slate-400 uppercase">Payment Amount</p><p className="text-xs font-bold text-emerald-600">₦{parseFloat(calculateTotalPayment()).toLocaleString()}</p></div>
